...
void LoadingScene3to4::AddLoadFunc(const function<void(void)>& func)
{
	mLoadList.push_back(func);
}

void LoadingScene3to4::Init()
{
	mLoadIndex = 0;
	mIsEndLoading = false;
	...
	//사운드 (BGM)
	AddLoadFunc([]() {SoundPlayer::GetInstance()->LoadFromFile(L"Scene4BGM", Resources(L"KIrbyBGM4.mp3"), true); });
...
void LoadingScene3to4::Render(HDC hdc)
{
