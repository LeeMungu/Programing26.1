...
void LoadingScene2to3::AddLoadFunc(const function<void(void)>& func)
{
	mLoadList.push_back(func);
}

void LoadingScene2to3::Init()
{
	mLoadIndex = 0;
	mIsEndLoading = false;
	...
	//사운드 (BGM)
	AddLoadFunc([]() {SoundPlayer::GetInstance()->LoadFromFile(L"Scene3BGM", Resources(L"KIrbyBGM3.mp3"), true); });
...
void LoadingScene2to3::Render(HDC hdc)
{
